...
def handle_event("validate", params, socket) do
  ...
  def handle_event("save", params, socket) do
    ...
    uploaded_files = consume_uploaded_entries(socket, :slide, fn %{path: path}, _entry -> dest = Path.join(["priv", "static", "uploads", Path.basename(path)])
        ...
        System.cmd("convert", [file_absolute_path, "-resize" ,"1080x1080", "-background" ,"#" <> params["background_color_hex"], "-gravity" ,"center", "-extent", "1080x1080" ,dest <>"/split/page-0%d.pdf"])
        IO.puts "done spliting"
        System.cmd("convert", ["-density" ,"150", dest <> "/split/*.pdf" ,dest <> "/output.pdf"])
